            ...
            <tbody>
                <?php if ($query) : ?>
                    <?php foreach ($query as $row) : ?>
                        <tr>
                            <td><?= $row->id ?></td>
                            <td><img class="img-thumbnail" src="<?= images_url("blogs/" . image("public/images/blogs/", $row->image)) ?>" alt="<?= $row->title ?>" width="70" height="70"></td>
                            <td><?= $row->slug ?></td>
                            <td><?= $row->title ?></td>
                            ...
                            <td style="width: 150px;">
                                <?php if ($this->auth->admin_perm_auth('blogs', 'edit')) : ?>
                                    <a href="<?= base_url("{$this->uri->segment(1)}/{$this->uri->segment(2)}/edit/{$row->id}") ?>" class="btn btn-sm btn-success"><i class="fas fa-edit"></i></a>
                                <?php endif ?>
                                <a href="<?= base_url("blog/{$row->slug}") ?>" class="btn btn-sm btn-info" target="_blank"><i class="fas fa-eye"></i></a>
                                <?php if ($this->auth->admin_perm_auth('blogs', 'delete')) : ?>
